                ...
                with tab_tk:
                    ...
                    st.markdown(f"""<div class="kpi-grid">
                      ...
                      <div class="kpi-card"><div class="kpi-label">Señal</div>
                        <div class="kpi-value" style="font-size:.8rem;color:{tend_c}">{_señal_ok}</div></div>
    ...
    if "precios" not in st.session_state:
        ...
        if btn_vf or any(k.startswith("vf_calc_") for k in st.session_state):
            ...
            for i_t,tkr in enumerate(tks_ok_vf):
                ...
                resultados_vf[tkr]={**datos,**res,
                    ...
                    "pot_pond":pot_p,"señal":señal,"señal_fcf":señal_fcf}
        ...
        if "resultados_vf" in st.session_state:
            ...
            for t,r in resultados_vf.items():
                ...
                precio_cv_r  = st.session_state.get(f"precio_conv_{t}", None)
                precio_act_r = r.get("precio",0) or 0
                if precio_cv_r and precio_act_r:
                    ratio_v = precio_cv_r / precio_act_r
                    señal_mult_r = ("🟢 COMPRAR" if ratio_v>1.50 else
                                   "🔴 VENDER"   if ratio_v<0.90 else "⚪ MANTENER")
                else:
                    ratio_v = np.nan
                    señal_mult_r = "⏳ Pendiente"
                css_ratio = ("td-positive" if (not np.isnan(ratio_v) and ratio_v>1.5)
                             else "td-negative" if (not np.isnan(ratio_v) and ratio_v<0.9)
                             else "td-neutral")
                rsr += ('<tr><td class="td-blue">'+str(t)+'</td>'
                        ...
                        +'<td style="font-weight:600">'+r.get("señal_fcf",r.get("señal","N/A"))+'</td>'
                        ...
                        +'<td style="font-weight:600">'+señal_mult_r+'</td>'
                ...
                with tab_vf_t:
                    ...
                    if not convergido:
                        ...
                        st.markdown(f"""<div style="background:#0D1E35;border:1px solid #1A3A5C;
                          ...
                          <span style="font-size:1.3rem;font-weight:600;color:{tend_c};">{_señal_ok}</span>
                        </div>""",unsafe_allow_html=True)
        else:
            st.markdown("""<div style="text-align:center;padding:4rem 2rem;color:#4A7FA5;
                 font-family:IBM Plex Mono,monospace;">
...
def _generar_pdf_fundamental(resultados_vf):
    ...
    import io, numpy as _np, datetime as _dt, os as _os

    # Registrar fuente con soporte para caracteres especiales (ñ, tildes)
    def _safe(txt):
        """Escapar chars especiales para ReportLab XML."""
    ...
    for tkr, r in resultados_vf.items():
        ...
        # Header band
        header_data = [[Paragraph(f"REPORTE DE VALORACION FUNDAMENTAL", S_TITLE)]]
        ...
        # Señal grande
        señal_box = Table([[Paragraph(f"Senal: {_safe(señal_m)}", sty("sb",
        ...
        if "port_calc" in st.session_state:
            ...
            story.append(Paragraph("SECCION 1 - PORTAFOLIO OPTIMO", S_H1))
            story.append(Spacer(1,0.1*inch))
            ...
            story.append(Paragraph("Composicion Optima del Portafolio", S_H2))
            ...
            if tks_port and pesos_p:
                story.append(Spacer(1,0.1*inch))
                story.append(Paragraph("Distribucion del Portafolio", S_H2))
                try:
        ...
        # ══════════════════════════════════════════════════════════════════════
        story.append(PageBreak())
        story.append(Paragraph("SECCION 2 - VALORACION POR FLUJO DE CAJA LIBRE", S_H1))
        story.append(Spacer(1,0.1*inch))

        col_left, col_right = [], []

        # FCF cálculo
        story.append(Paragraph("Calculo del FCF", S_H2))
        ...
        # Proyección
        story.append(Paragraph("Proyeccion FCF", S_H2))
        proyt = r.get("proyt",[])
        if proyt:
        ...
        sfb = Table([[Paragraph(f"Senal FCF: {_safe(str(señal_f))}", sty("sf",
        ...
        # ══════════════════════════════════════════════════════════════════════
        story.append(PageBreak())
        story.append(Paragraph("SECCION 3 - VALORACION POR MULTIPLES COMPARABLES", S_H1))
        story.append(Spacer(1,0.1*inch))
        ...
        # Precio sugerido
        story.append(Paragraph("Precio Sugerido por Multiples (Convergido)", S_H2))
        ...
        # Señal múltiplos
        smb = Table([[Paragraph(f"Senal Multiples: {_safe(str(señal_m))}", sty("sm",
        ...
        # ══════════════════════════════════════════════════════════════════════
        story.append(PageBreak())
        story.append(Paragraph("CONCLUSION Y RECOMENDACION", S_H1))
        story.append(Spacer(1,0.15*inch))
        ...
        conclusion = (
            f"El analisis fundamental de <b>{_safe(tkr)}</b> combina dos metodologias complementarias. "
            f"El modelo DCF arroja un precio objetivo de <b>{_fv(po_usd,'usd')}</b> "
            f"con un potencial de <b>{pot_fcf_s}</b>, utilizando un WACC de <b>{_fv(wacc_v,'pct')}</b>. "
            f"La valoracion por multiples comparables converge en un precio de <b>{_fv(precio_cv,'usd')}</b>, "
            f"representando el <b>{ratio_s}</b> del precio actual. "
            f"Senal consolidada: <b>{_safe(señal_m)}</b>."
        )
        story.append(Paragraph(conclusion, S_BODY))
        story.append(Spacer(1,0.15*inch))

        # Señal final grande
        sfinal = Table([[Paragraph(f"SENAL FINAL: {_safe(str(señal_m))}", sty("sfinal",
